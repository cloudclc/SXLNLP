...
class DataGenerator:
    ...
    #依照一定概率生成负样本或正样本
    #负样本从随机两个不同的标准问题中各随机选取两个个
    #正样本从随机一个标准问题中随机选取三个
    def random_train_sample(self):
        standard_question_index = list(self.knwb.keys())
        # 随机正样本
        if self.config["type"] == "cosine_embedding":
            if random.random() <= self.config["positive_sample_rate"]:
                ...
                return [s1, s2, torch.LongTensor([-1])]
        elif self.config["type"] == "cosine_triplet":
            # 随机两个正样本，一个负样本
            p, n = random.sample(standard_question_index, 2)
            # 如果选取到的标准问下不足两个问题，则无法选取，所以重新随机一次
            if len(self.knwb[p]) < 2:
                ...
                a, p = random.sample(self.knwb[p], 2)  # a是锚点，p是正样本
                n = random.choice(self.knwb[n])  # n是负样本
                return [a, p, n]
